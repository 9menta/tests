        ...
        if player ~= localPlayer then
            ...
            if character and character:FindFirstChild("HumanoidRootPart") then
                ...
                if distance > ESP_SETTINGS.General.MaxDistance then
                    -- Esconder todos os elementos da ESP
                    for _, drawing in pairs(esp) do
                        if type(drawing) ~= "table" then
                ...
                -- Se passou por todas as verificações, atualizar a ESP
                ...
                -- Se o personagem não existe ou não tem HumanoidRootPart
                for _, drawing in pairs(esp) do
                    if type(drawing) ~= "table" then
                        drawing.Visible = false
                    else
...
-- Função para limpar completamente a ESP
...
-- Modificar o evento de mudança de round
local function onRoundStateChanged()
    clearEsp() -- Limpar toda a ESP primeiro
    task.wait(1) -- Esperar um pouco para os personagens carregarem
